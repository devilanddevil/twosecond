 ...
 * TwoSecond_TSjsMin
 *
 * Usage - TwoSecond_TSjsMin::minify($js);
 * Usage - TwoSecond_TSjsMin::minify($js, $options);
 * Usage - TwoSecond_TSjsMin::minify($js, array('flaggedComments' => false));
 *
 * @package JShrink
 * @author Robert Hafner <tedivm@tedivm.com>
 * @license http://www.opensource.org/licenses/bsd-license.php  BSD License
 */
class TwoSecond_TSjsMin
{
    /**
     * Heuristic check whether a JavaScript string appears to be already minified.
     *
     * This does not guarantee correctness, but aims to avoid re-minifying
     * production bundles by examining common signals:
     * - Very long lines
     ...
     * @return bool
     */
    public static function isLikelyMinified($js)
    {
        if ($js === null || $js === '') {
            return false;
        }
        ...
        $minified = preg_replace('/\s+/', '', $normalized);
        $minifiedLength = strlen($minified);
        $ratio = $minifiedLength / $length;

        // If minified length is less than 90% of original, it's NOT minified
        if ($ratio >= 0.9) {
            return true;
        }

        // Break into lines (cap to avoid memory blowups on huge single-line files)
        $lines = explode("\n", $normalized);
        ...
        for ($i = 0; $i < $numLines; $i++) {
            ...
            // Detect typical indentation (2+ spaces or a tab before a non-space)
            if (preg_match('/^(\s{2,}|\t)\S/', $line) === 1) {
                $indentedLines++;
            }
        }

        $avgLineLen = $totalLineLen / $numLines;

        // Whitespace ratio: spaces and tabs vs total
        $onlyWhitespace = preg_replace('/[^\t\x20]/', '', $normalized);
        $whitespaceRatio = strlen($onlyWhitespace) / $length;

        // Comment presence
        $hasBlockComment = preg_match('/\/\*/', $normalized) === 1;
        $hasLineComment  = preg_match('/(^|[^:])\/\//m', $normalized) === 1; // avoid matching URLs
        $hasComments = $hasBlockComment || $hasLineComment;

        // Space after control keywords (pretty code often has a space)
        $hasKeywordSpace = preg_match('/\b(function|if|for|while|switch|catch)\s+\(/', $normalized) === 1;

        // Semicolon-newline density (pretty code often breaks lines after ;) 
        $semicolonNewline = preg_match_all('/;\s*\n/', $normalized);

        // Build a simple score
        $score = 0;
        if ($maxLineLen >= 500) $score++;
        if ($avgLineLen >= 120) $score++;
        if ($whitespaceRatio <= 0.20) $score++;
        if ($indentedLines <= ($numLines * 0.05)) $score++;
        if (!$hasComments) $score++;
        if (!$hasKeywordSpace) $score++;
        if ($semicolonNewline <= ($numLines * 0.01)) $score++;

        // Threshold: 4+ signals => likely minified
        return $score >= 4;
    }
    ...
    protected function loop()
    {
        while ($this->a !== false && !is_null($this->a) && $this->a !== '') {
            ...
            if ($this->b == '/') {
                // Treat potential unquoted '/' as a regex delimiter only after
                // a safe set of tokens. Excluding ':' here prevents false
                // positives on URL strings like "https://" that may appear
                // after object-literal keys (e.g., key:"https://...").
                // This trades off extremely rare cases of object values that
                // begin with a regex literal immediately after ':' without any
                // whitespace, which is uncommon and still handled when other
                // heuristics (keywords) apply.
                $valid_tokens = "(,=[!&|?\n";
    ...
    protected function processMultiLineComments($startIndex)
    {
        ...
        if ($char === false) {
            throw new \RuntimeException('Unclosed multiline comment');
        }
